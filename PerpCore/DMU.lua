	...
	
	{
		...
		
		{
			data = 
			{
				...
				enabled = false,
				eventType = 4,
				execute = "-- [Debug] OnEntityMarkerAdd\n-- Verbose logging of every marker added to an entity. Loops (self.used each fire).\n\nlocal ent = TensorCore.mGetEntity(eventArgs.entityID or eventArgs.entityId)\n\nd(\"[DMU Marker] markerID=\" .. tostring(eventArgs.markerID) ..\n    \" entityID=\" .. tostring(eventArgs.entityID) ..\n    \" name=\" .. tostring(ent and ent.name or \"?\") ..\n    \" cid=\" .. tostring(ent and ent.contentid or \"?\") ..\n    \" type=\" .. tostring(ent and ent.type or \"?\") ..\n    (ent and ent.pos and (\" pos=\" .. string.format(\"%.1f, %.1f, %.1f\", ent.pos.x, ent.pos.y, ent.pos.z)) or \"\"))\n\nself.used = true\n",
				executeType = 2,
				loop = true,
				mechanicTime = 15.282,
		...
		
		{
			data = 
			{
				...
				enabled = false,
				eventType = 3,
				execute = "-- [Debug] OnEntityChannel\n-- Verbose logging of NPC channels only. Loops (self.used each fire).\n-- eventArgs is a userdata object (pairs() returns nothing), so fields are read explicitly.\n\nself.used = true\n\nif not eventArgs then\n    return\nend\n\nlocal casterId = eventArgs.entityID or eventArgs.entityId or eventArgs.sourceEntityID\n\nlocal function casterIsPartyMember(id)\n    if not id then\n        return false\n    end\n\n    if PerpCore and PerpCore.GetPlayer then\n        local me = PerpCore.GetPlayer(data and data.perspective)\n        if me and me.id == id then\n            return true\n        end\n    end\n\n    if PerpCore and PerpCore.GetPartyEntities then\n        for _, m in ipairs(PerpCore.GetPartyEntities() or {}) do\n            if m and m.id == id then\n                return true\n            end\n        end\n    end\n\n    local caster = TensorCore and TensorCore.mGetEntity and TensorCore.mGetEntity(id)\n    if not caster or not caster.name or caster.name == \"\" then\n        return false\n    end\n\n    if PerpCore and PerpCore.Config and PerpCore.Config.PartyRoles then\n        for _, roleName in pairs(PerpCore.Config.PartyRoles) do\n            if roleName ~= \"\" and string.find(caster.name, roleName, 1, true) then\n                return true\n            end\n        end\n    end\n\n    if PerpCore and PerpCore.GetPartyEntities then\n        for _, m in ipairs(PerpCore.GetPartyEntities() or {}) do\n            if m and m.name and m.name ~= \"\" and string.find(caster.name, m.name, 1, true) then\n                return true\n            end\n        end\n    end\n\n    return false\nend\n\nif casterIsPartyMember(casterId) then\n    return\nend\n\nlocal caster   = casterId and TensorCore.mGetEntity(casterId)\nlocal targetId = eventArgs.targetID or eventArgs.targetId or eventArgs.mainTargetID\nlocal target   = targetId and TensorCore.mGetEntity(targetId)\nlocal spellID  = eventArgs.spellID or eventArgs.spellId or eventArgs.actionID\n\nlocal spellName = \"?\"\nif spellID and ActionList and ActionList.Get then\n    local ok, action = pcall(function() return ActionList:Get(1, spellID) end)\n    if ok and action and action.name then\n        spellName = action.name\n    end\nend\n\nd(\"[DMU Channel] spell=\" .. tostring(spellName) .. \" (id \" .. tostring(spellID) .. \")\" ..\n    \" caster=\" .. tostring(caster and caster.name or \"?\") ..\n    \" (id \" .. tostring(casterId) .. \", cid \" .. tostring(caster and caster.contentid) ..\n    \", type \" .. tostring(caster and caster.type) .. \")\" ..\n    \" target=\" .. tostring(target and target.name or \"?\") .. \" (id \" .. tostring(targetId) .. \")\" ..\n    \" heading=\" .. tostring(eventArgs.heading) ..\n    (eventArgs.castPosX and (\" castPos=\" .. string.format(\"%.1f, %.1f, %.1f\",\n        eventArgs.castPosX, eventArgs.castPosY or 0, eventArgs.castPosZ or 0)) or \"\"))\n",
				executeType = 2,
				loop = true,
				mechanicTime = 15.282,
		...
		
		{
			data = 
			{
				...
				enabled = false,
				eventType = 19,
				execute = "-- [Debug] OnEventObjectScript\n-- Verbose logging of event-object scripts. Loops (self.used each fire).\n-- Raw callback is function(entityID, a2, a3, a4). The TensorReactions field names for the extra\n-- args aren't documented, and eventArgs is userdata (pairs() returns nothing), so we probe a wide\n-- set of candidate keys and print whichever come back non-nil to discover the real names.\n\nlocal ENTITY_TYPE = {\n    [0] = \"None\",\n    [1] = \"Player\",\n    [2] = \"BNpc\",\n    [3] = \"Npc\",\n    [4] = \"Mount\",\n    [5] = \"Companion\",\n    [6] = \"Retainer\",\n    [7] = \"EventObj\",\n    [8] = \"Treasure\",\n    [9] = \"Aetheryte\",\n}\n\nlocal function typeName(t)\n    return ENTITY_TYPE[tonumber(t)] or (\"type=\" .. tostring(t))\nend\n\nlocal function fmtPos(pos)\n    if not pos then return \"?\" end\n    return string.format(\"%.1f, %.1f, %.1f\", pos.x or 0, pos.y or 0, pos.z or 0)\nend\n\nlocal function fmtHeading(rad)\n    if rad == nil then return \"?\" end\n    return string.format(\"%.1f deg (%.3f rad)\", math.deg(rad), rad)\nend\n\nlocal function distToPlayer(ent)\n    if not (ent and ent.pos and TensorCore and TensorCore.mGetPlayer) then return end\n    local me = TensorCore.mGetPlayer()\n    if not (me and me.pos) then return end\n    if TensorCore.getDistance2d then\n        return TensorCore.getDistance2d(me.pos, ent.pos)\n    end\n    local dx, dz = (ent.pos.x or 0) - me.pos.x, (ent.pos.z or 0) - me.pos.z\n    return math.sqrt(dx * dx + dz * dz)\nend\n\nif not eventArgs then\n    d(\"[DMU EObjScript] <no eventArgs>\")\n    self.used = true\n    return\nend\n\n-- Candidate keys for the entity and the three generic script args (a2/a3/a4).\nlocal candidates = {\n    \"entityID\", \"entityId\", \"sourceEntityID\", \"targetID\", \"targetId\",\n    \"a1\", \"a2\", \"a3\", \"a4\",\n    \"arg1\", \"arg2\", \"arg3\", \"arg4\",\n    \"param1\", \"param2\", \"param3\", \"param4\",\n    \"data1\", \"data2\", \"data3\", \"data4\",\n    \"value\", \"value1\", \"value2\", \"value3\",\n    \"intValue\", \"intValue1\", \"intValue2\", \"intValue3\",\n    \"scriptID\", \"eventID\", \"eobjID\", \"state\", \"flags\", \"type\",\n    \"spellID\", \"actionID\", \"markerID\", \"heading\",\n}\n\nlocal parts = {}\nfor _, key in ipairs(candidates) do\n    local v = eventArgs[key]\n    if v ~= nil then\n        parts[#parts + 1] = key .. \"=\" .. tostring(v)\n    end\nend\n\nd(\"[DMU EObjScript] eventArgs: \" .. (#parts > 0 and table.concat(parts, \", \") or \"<no known fields found>\"))\n\n-- Resolve entity id and script args (raw callback: entityID, a2, a3, a4).\nlocal entId = eventArgs.entityID or eventArgs.entityId or eventArgs.sourceEntityID\n    or eventArgs.a1 or eventArgs.arg1 or eventArgs.param1\nlocal a2 = eventArgs.a2 or eventArgs.arg2 or eventArgs.param2 or eventArgs.data2\nlocal a3 = eventArgs.a3 or eventArgs.arg3 or eventArgs.param3 or eventArgs.data3\nlocal a4 = eventArgs.a4 or eventArgs.arg4 or eventArgs.param4 or eventArgs.data4\n\nd(\"[DMU EObjScript] resolved entityID=\" .. tostring(entId) ..\n    \" a2=\" .. tostring(a2) ..\n    \" a3=\" .. tostring(a3) ..\n    \" a4=\" .. tostring(a4))\n\nlocal ent = entId and TensorCore and TensorCore.mGetEntity and TensorCore.mGetEntity(entId)\nif ent then\n    local dist = distToPlayer(ent)\n    d(\"[DMU EObjScript]   entity: name=\" .. tostring(ent.name) ..\n        \" id=\" .. tostring(ent.id) ..\n        \" cid=\" .. tostring(ent.contentid) ..\n        \" bnpc=\" .. tostring(ent.bnpcid) ..\n        \" type=\" .. typeName(ent.type) ..\n        (ent.pos and (\" pos=\" .. fmtPos(ent.pos)) or \"\") ..\n        (ent.heading ~= nil and (\" heading=\" .. fmtHeading(ent.heading)) or \"\") ..\n        (dist and (\" dist=\" .. string.format(\"%.1f\", dist)) or \"\"))\n\n    if ent.contentid then\n        d(\"[DMU EObjScript]   cond hint: contentid==\" .. tostring(ent.contentid))\n    end\nelseif entId then\n    d(\"[DMU EObjScript]   entity: mGetEntity(\" .. tostring(entId) .. \") returned nil\")\nend\n\n-- If a2/a3/a4 look like entity ids, resolve them too (some scripts pass targets).\nfor _, label in ipairs({ \"a2\", \"a3\", \"a4\" }) do\n    local id = ({ a2 = a2, a3 = a3, a4 = a4 })[label]\n    if id and tonumber(id) and tonumber(id) ~= tonumber(entId) and TensorCore and TensorCore.mGetEntity then\n        local ref = TensorCore.mGetEntity(id)\n        if ref then\n            d(\"[DMU EObjScript]   \" .. label .. \" entity: name=\" .. tostring(ref.name) ..\n                \" id=\" .. tostring(ref.id) ..\n                \" cid=\" .. tostring(ref.contentid) ..\n                (ref.pos and (\" pos=\" .. fmtPos(ref.pos)) or \"\"))\n        end\n    end\nend\n\nself.used = true\n",
				executeType = 2,
				loop = true,
				mechanicTime = 15.282,
			...
			data = 
			{
				actions = 
				{
					
					{
						...
						{
							aType = "Lua",
							actionLua = "-- Trine Draw AOEs\n-- OnEventObjectScript\n-- Entity position = center of the trine triangle. Corner offsets derived from the reference layout below.\n\nlocal DRAW_MS          = 10530\nlocal RADIUS           = 6\nlocal SEG              = 48\nlocal oldDraws         = true\n\nlocal DORITO_UP_CID    = 2015154\nlocal DORITO_DOWN_CID  = 2015155\n\n-- Measured reference triangle (instance coords).\nlocal REF_CENTER       = { x = 102.9, y = 0, z = 85 }\nlocal REF_TOP          = { x = 100.0, y = 0, z = 80 }\nlocal REF_BOTTOM_LEFT  = { x = 100.0, y = 0, z = 90 }\nlocal REF_BOTTOM_RIGHT = { x = 108.0, y = 0, z = 85 }\n\nlocal CORNER_OFFSETS   = {\n    top = {\n        x = REF_TOP.x - REF_CENTER.x,\n        y = REF_TOP.y - REF_CENTER.y,\n        z = REF_TOP.z - REF_CENTER.z,\n    },\n    bottomLeft = {\n        x = REF_BOTTOM_LEFT.x - REF_CENTER.x,\n        y = REF_BOTTOM_LEFT.y - REF_CENTER.y,\n        z = REF_BOTTOM_LEFT.z - REF_CENTER.z,\n    },\n    bottomRight = {\n        x = REF_BOTTOM_RIGHT.x - REF_CENTER.x,\n        y = REF_BOTTOM_RIGHT.y - REF_CENTER.y,\n        z = REF_BOTTOM_RIGHT.z - REF_CENTER.z,\n    },\n}\n\n-- Return the three corner world positions for a trine centered at (cx, cy, cz).\n-- flip180 = true rotates the triangle 180° around the center (dorito down).\nfunction getTrineCornerPositions(cx, cy, cz, flip180)\n    local o = CORNER_OFFSETS\n    local sx, sz = flip180 and -1 or 1, flip180 and -1 or 1\n    return {\n        { x = cx + o.top.x * sx,         y = cy + o.top.y,         z = cz + o.top.z * sz },\n        { x = cx + o.bottomLeft.x * sx,  y = cy + o.bottomLeft.y,  z = cz + o.bottomLeft.z * sz },\n        { x = cx + o.bottomRight.x * sx, y = cy + o.bottomRight.y, z = cz + o.bottomRight.z * sz },\n    }\nend\n\n-- Draw radius-6 circles at each trine corner for DRAW_MS milliseconds.\n-- colorStart -> colorEnd shifts over the full draw duration (orange -> bright red).\nfunction drawTrineAOEsAtCenter(cx, cy, cz, flip180, ms, radius, colorStart, colorEnd)\n    if not (Argus2 and Argus2.addTimedCircleFilled and GUI) then return end\n    ms         = ms or DRAW_MS\n    radius     = radius or RADIUS\n    colorStart = colorStart or GUI:ColorConvertFloat4ToU32(1, 0.55, 0.05, 0.5) -- orange\n    colorEnd   = colorEnd or GUI:ColorConvertFloat4ToU32(1, 0.05, 0.05, 0.9)   -- bright red\n    local whiteOutline = GUI:ColorConvertFloat4ToU32(1, 1, 1, 1)\n\n    for _, corner in ipairs(getTrineCornerPositions(cx, cy, cz, flip180)) do\n        Argus2.addTimedCircleFilled(\n            ms, corner.x, corner.y, corner.z,\n            radius, SEG, colorStart, colorEnd,\n            nil, 0, nil, whiteOutline, 1.0, 3, nil, oldDraws\n        )\n        if AnyoneCore and AnyoneCore.addWorldTextCountdown then\n            AnyoneCore.addWorldTextCountdown(\n                ms,\n                { x = corner.x, y = corner.y + 1.2, z = corner.z },\n                AnyoneCore.white,\n                true,\n                1.0\n            )\n        end\n    end\nend\n\n-- —— reaction body (entity id / cid validated by reaction condition) ——\n\nlocal entId = eventArgs and (eventArgs.entityID or eventArgs.entityId)\nlocal ent   = entId and TensorCore and TensorCore.mGetEntity and TensorCore.mGetEntity(entId)\nif not (ent and ent.pos) then\n    self.used = true\n    return\nend\n\nlocal cid = tonumber(ent.contentid)\nlocal flip180\nif cid == DORITO_UP_CID then\n    flip180 = false\nelseif cid == DORITO_DOWN_CID then\n    flip180 = true\nelse\n    return\nend\n\nlocal cx, cy, cz = ent.pos.x, ent.pos.y, ent.pos.z\nif not cy or cy == 0 then\n    cy = (data and data.arenaCenter and data.arenaCenter.y) or 0\nend\n\ndrawTrineAOEsAtCenter(cx, cy, cz, flip180)\n\nself.used = true\n",
							conditions = 
							{
								
								{
									"0621b20f-c85b-e7b8-9166-22166e482178",
									true,
								},
								
								{
									"d6ba079c-3274-987a-9be3-06999b4a3374",
									true,
								},
								
								{
									"c3212723-8090-1d42-b278-357340e126c8",
									true,
								},
								
								{
									"a6cd0d40-ab4a-7658-92fe-323a33c5e53b",
									true,
								},
							},
							gVar = "ACR_TensorMagnum3_CD",
							uuid = "8f0fd373-84f0-097c-a9ce-002f3edc7038",
							version = 2.1,
						},
					},
				},
				conditions = 
				{
					
					{
						...
						{
							category = "Lua",
							conditionLua = "local entId = eventArgs.entityID or eventArgs.entityId or eventArgs.sourceEntityID\n    or eventArgs.a1 or eventArgs.arg1 or eventArgs.param1\nlocal ent = entId and TensorCore and TensorCore.mGetEntity and TensorCore.mGetEntity(entId)\nreturn ent ~= nil and (ent.contentid == 2015154 or ent.contentid == 2015155)",
							name = "Is Trine",
							uuid = "0621b20f-c85b-e7b8-9166-22166e482178",
							version = 3,
						},
						inheritedIndex = 1,
					},
					
					{
						data = 
						{
							...
							eventIntValue = 16,
							name = "a2",
							uuid = "d6ba079c-3274-987a-9be3-06999b4a3374",
							version = 3,
						},
					},
					
					{
						data = 
						{
							...
							eventIntValue = 32,
							name = "a3",
							uuid = "c3212723-8090-1d42-b278-357340e126c8",
							version = 3,
						},
					},
					
					{
						data = 
						{
							...
							eventArgType = 4,
							name = "a3",
							uuid = "a6cd0d40-ab4a-7658-92fe-323a33c5e53b",
							version = 3,
						},
					},
				},
				eventType = 19,
				loop = true,
				mechanicTime = 348.873,
				name = "[Trine] Draw AOEs",
				timeRange = true,
				timelineIndex = 66,
				timerEndOffset = 30,
				timerStartOffset = -5,
				uuid = "2c5b5dc2-57b4-d326-909a-8efb66b60d9a",
				version = 2,
			},
		},
	},
